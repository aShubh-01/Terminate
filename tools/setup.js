        ...
        tty.on('keypress', (str, key) => {
            ...
            if (key.name === 'up') {
                ...
                if (selected.size > 0) {
                    ...
                    require('fs').writeFileSync('/tmp/.termination_shells', result);
                    resolve(result);
...
/**
 * Authenticate the user by storing a master encryption key
 * @param {object} args
 * @param {string} args.key - Secret key
 */
exports.login = async ({ key }) => {
  const fs = require('fs');
  const path = require('path');
  try {
    const configPath = path.join(config.corePath, "config.json")
    ...
    // Store the key in config.json as the encryptionKey
    currentConfig.encryptionKey = key
    fs.writeFileSync(configPath, JSON.stringify(currentConfig, null, 2))
    
    return "SUCCESS: Encryption key configured."
  } catch (err) {
